class FIRSTTeam {
    ...
    constructor(event, tbaInfo) {
        ...
        const YearCollectionClass = MatchScoutingCollection[event.info.year];
        const CollectionClass = YearCollectionClass ? YearCollectionClass : MatchScoutingCollection;
        /**
     ...
     */
    get matchesObj() {
        return this.matches.reduce((obj, match) => {
            obj[match.key] = match;
            return obj;
        }, {});
    }
     ...
     */
    addMatchScouting(...scoutings) {
        if (!MatchScouting[this.event.info.year]) throw new Error('Invalid year for match scouting: ' + this.event.info.year);

        scoutings.forEach((s, i) => {
            if (!s instanceof MatchScouting) throw new Error('Invalid scouting type in index ' + i + ' of scoutings array');
        });
        ...
        scoutings.forEach(s => {
            ...
            try {
                if ((s.compLevel == "pr" || s.preScoutingKey) || (match && match.hasTeam(this.number))) {
                    const scouting = new MatchScouting[this.event.info.year](s, match);
    ...
    get frameDimensions () {
        ...
        const { Dimensions, Width, Length } = data;
